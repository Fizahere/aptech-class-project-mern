...
const CustomForm = () => {
    ...
    validate: (values) => {
      ...
      if (!token) {
        ...
        if (!values.password) {
          ...
          errors.password = "password must be at least 5 charpcters long.";
        }
        return errors;
      }
    ...
    onSubmit: (values) => {
      async function send_data() {
        try {
          ...
          setMessagae(`Welcome ${response.data.user}`);
          formik.resetForm();
        } catch (error) {
  ...
  async function check_login() {
    try {
      ...
      setMessagae(`you're loggedIn ${response.data.user}`);
    } catch (error) {
      setMessagae("you aren't loggedIn.");
    }
  }
      ...
      <form onSubmit={formik.handleSubmit}>
        <div className="form">
          ...
          <div className="password-field">
            ...
            <i className="icon">
              {showPassword ? (
                <IoMdEyeOff onClick={toglePasswordIcon} />
              ) : (
                <IoMdEye onClick={toglePasswordIcon} />
              )}
            </i>
          </div>
          {formik.touched.password && formik.errors.password ? (
